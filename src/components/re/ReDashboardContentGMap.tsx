...
export default function ReDashboardContentGMap({}) {
  ...
  useEffect(() => {
    fetch("/data/pennsylvania_re_data.geojson")
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    const loadMarkers = async () => {
      ...
      gMap.current.data.setStyle((feature) => {
        ...
        let color = "#000000";
        if (technology === "Solar") color = "hsla(18, 76%, 51%, 0.8)";
        if (technology === "Wind") color = "hsla(245, 85%, 41%, 0.57)";
  ...
  const getCurrentCf = (feature: google.maps.Data.Feature) => {
    const tech = feature.getProperty("technology");
    const current_cf = (feature.getProperty("current_cf") as number) || 0;

    if (tech === "Wind") {
      return (current_cf * 100 * 0.75).toFixed(2);
    }
    if (tech === "Solar") {
      return (current_cf * 0.9 * 100).toFixed(2);
    }
  ...
  const getPopupContent = (feature: google.maps.Data.Feature) => {
    ...
    const ownerName = feature.getProperty("owner") || "N/A";
    const capacityMw =
      (feature.getProperty("capacity_mw") as number).toFixed(1) || 0;
    const optimalSolarMw =
      (feature.getProperty("optimal_solar_mw") as number).toFixed(1) || 0;
    const optimalWindMw =
      (feature.getProperty("optimal_wind_mw") as number).toFixed(1) || 0;
    const optimalBatteryMw =
      (feature.getProperty("optimal_battery_mw") as number).toFixed(1) || 0;
    const optimalBatteryMwh =
      (feature.getProperty("optimal_battery_mwh") as number).toFixed(1) || 0;
    const currentCf = getCurrentCf(feature) || 0;
    const optimalCf =
          ...
          <div id="overview-tab" class="tab-content" style="display: block;">
            <div class="info-grid">
              ...
              <div class="info-row">
                ...
                <span class="info-value">${capacityMw} MW</span>
              </div>
            </div>
          </div>
          ...
          <div id="surplus-tab" class="tab-content" style="display: none;">
            <div class="info-grid">
              <div class="info-row">
                ...
                <span class="info-value">${optimalSolarMw} MW</span>
              </div>
              <div class="info-row">
                <span class="info-label">Wind Integration Potential:</span>
                <span class="info-value">${optimalWindMw} MW</span>
              </div>
              <div class="info-row">
                <span class="info-label">Storage Integration Potential:</span>
                <span class="info-value">${optimalBatteryMw} MW</span>
              </div>
              <div class="info-row">
                <span class="info-label">Storage Integration Potential:</span>
  ...
  const handleFilterChange = (filterName: string, value: string | boolean) => {
    ...
    if (filterName === "county") {
      delete newFilters.utility_name;
      delete newFilters.plant_name;
    }
    if (filterName === "utility_name") {
      delete newFilters.plant_name;
    }
  ...
  const loadVisualLayer = async () => {
    ...
    const s3Url = `https://etaiplatform.s3.us-west-2.amazonaws.com/existing_re_updated/${selectedPlant.technology.toLowerCase()}/facility_siting_outputs/visual_geojson/${
